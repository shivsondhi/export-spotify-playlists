...
csv_headers = ["Artist","Album","TrackName", "Label"]

# whether you want to overwrite existing files or not
OVERWRITE = True


def write_playlist(username, uri, mode, cli_id=CLI_ID, cli_key=CLI_KEY, token=None):
	'''
	...
	'''
	global spotify
	playlist_df = pd.DataFrame(columns = csv_headers)
	...
	if mode == 'csv':
		...
		old_total = write_csv(filename, playlist_df, webapp) 
	else:
...
def write_csv(filename, playlist_df, webapp):
	'''
	ADD TO CSV FILE
